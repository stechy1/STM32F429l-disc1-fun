...
//Faster then LCD_ILI9341_DrawPixel, but different header
void LCD_ILI9341_DrawPixelEx(uint16_t x, uint16_t y, uint8_t colorMSB,uint8_t colorLSB )
{
	LCD_ILI9341_SetCursorPosition(x, y, x, y);
	LCD_ILI9341_SendCommand(ILI9341_GRAM);
	LCD_ILI9341_SendData(colorMSB);
	LCD_ILI9341_SendData(colorLSB);
}
...
void LCD_ILI9341_DisplayImage(uint16_t image[ILI9341_PIXEL])
{
	...
	for (n = 0; n < ILI9341_PIXEL; ++n)
  {
    data = image[n];
		...
		LCD_SPI_Send(ILI9341_SPI, (uint8_t) data);
	}
...
void LCD_ILI9341_Putc(uint16_t x, uint16_t y, char c, LCD_FontDef_t *font, uint16_t foreground, uint16_t background) {
	uint32_t i, b, j, tmp;
  uint8_t colorForMSB, colorForLSB, colorBackMSB, colorBackLSB;

  colorForMSB =  foreground >> 8;
  colorForLSB = foreground & 0xFF;

  colorBackMSB =  background >> 8;
  colorBackLSB = background & 0xFF;
	/* Set coordinates */
	ILI9341_x = x;
	ILI9341_y = y;
	...
	for (i = 0; i < font->FontHeight; ++i)
  {
		...
		for (j = 0; j < font->FontWidth; ++j)
    {
			if ((b << j) & 0x8000)
      {
				LCD_ILI9341_DrawPixelEx(ILI9341_x + j, (ILI9341_y + i), colorForMSB,colorForLSB);
			}
      else
      {
				LCD_ILI9341_DrawPixelEx(ILI9341_x + j, (ILI9341_y + i), colorBackMSB,colorBackLSB);
			}
		}
	}
...
void LCD_ILI9341_DrawLine(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t color) {
  ...
  colorLSB = color & 0xFF;
	...
	while (1)
  {
		LCD_ILI9341_DrawPixelEx(x0, y0, colorMSB, colorLSB);
		if ((x0 == x1) && (y0 == y1))
    {
			break;
		}
