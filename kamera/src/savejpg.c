...
void fdct_and_quantization(SBYTE *data,float *fdtbl,SWORD *outdata)
{
  ...
  for (ctr = 7; ctr >= 0; ctr--)
  {
    ...
    z5 = ((tmp10 - tmp12) * 392) >> 10;

    //z2 = ((float) 0.541196100) * tmp10 + z5; // c2-c6
    z2 = ((tmp10* 554) >> 10) + z5; // c2-c6
    //z4 = ((float) 1.306562965) * tmp12 + z5; // c2+c6
    z4 = ((tmp12 * 1337) >> 10) + z5; // c2+c6
...
// Apply the quantization and scaling factor
    outdata[i] =  (int)(datafloat[i] * fdtbl[i]);
...
void load_data_units_from_RGB_buffer(WORD xpos,WORD ypos)
{
  ...
  for (y=0; y<8; y++)
  {
    for (x=0; x<8; x++)
    {
      G=RGB_buffer[location].R;
      B=RGB_buffer[location].G;
      R=RGB_buffer[location].B;
      YDU[pos]=Y(R,G,B);
      CbDU[pos]=Cb(R,G,B);
      CrDU[pos]=Cr(R,G,B);
      location++;
...
// Load BMP image from disk and complete X
  ...
  RGB_buffer = (colorRGB*) SDRAM_RGB888;
